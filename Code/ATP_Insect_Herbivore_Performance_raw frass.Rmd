...
df_DFE1 = subset(DFE1, select = -c(percent.em1)) #drop duplicated column 

percent.em2 <- DFE2$flies2/DFE2$pupae2
DFE2 <- cbind(percent.em2, DFE2)
DFE2
df_DFE2 = subset(DFE2, select = -c(percent.em2)) #drop duplicated column 
...
lglm.DFE2 <- glm(percent.em2 ~ amendment2, data = DFE2, family = "binomial", weights = pupae2) # used here for consistency with lglm.DFE1
gamma.DFE2 <- glm(percent.em2 ~ amendment2, family = Gamma(link = "log"), data = DFE2) # gamma.DFE2, small AIC but no better result
...
**DPR1, DPR2, DPW1, DFE1 show significant differences** *DPW2 is only marginally significant (p=0.077)*
   ...
   { for (j in (i+1):N)
    {
     ...
     critical.range = c(critical.range,
      sqrt(qchisq(.95,2))*sqrt(p[i]*(1-p[i])/300 + p[j]*(1-p[j])/300))
    }
   }
...
ggsave("boxplot_P26.jpeg", width = 5, height = 4, dpi = 1000)
