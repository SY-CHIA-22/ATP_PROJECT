...
PPR2$date2 <- as.factor(PPR2$date2)
...
plot_lp<-lapply(delia.list[1:2], function(part_of_list) # phrase "part-of-list" can be anything
 ggplot(data = part_of_list, aes(x = amendment, y = tot_larv_pupa))+ geom_boxplot())
plot_lp
...
plot_l_pup<-lapply(plutella.list[1:2], function(part_of_list) # phrase "part-of-list" can be anything
 ggplot(data = part_of_list, aes(x = amendment, y = pupae))+ geom_boxplot())
plot_l_pup
...
glm_out <- glm(tot_larv_pupa ~ amendment, data = x, family = quasipoisson)
...
glm_output <- glm(tot_larv_pupa ~ amendment, data = x, family = poisson)
...
qpois.PPR1 <- glm(pupae1 ~ amendment1, family = quasipoisson, data = PPR1) # overdispersion
glm.PPR1 <- glm(pupae1 ~ amendment1, data = PPR1)
nbin.PPR1 <- glm.nb(pupae1 ~ amendment1, data = PPR1)
AIC(lm.PPR1, qpois.PPR1, glm.PPR1, nbin.PPR1)

# Check overdispersion. The ratio should be around 1
Overdisp_fun(qpois.PPR1)

lm.PPR2 <- lm(pupae2 ~ amendment2, data = PPR2)
qpois.PPR2 <- glm(pupae2 ~ amendment2, family = quasipoisson, data = PPR2) #overdispersion
glm.PPR2 <- glm(pupae2 ~ amendment2, data = PPR2)
nbin.PPR2 <- glm.nb(pupae2 ~ amendment2, data = PPR2)
AIC(lm.PPR2, qpois.PPR2, glm.PPR2, nbin.PPR2)
...
nbin.output <- glm.nb(pupae ~ amendment, data = xp) # xp can be anything
