...
gamma.DFE1 <- glm(percent.em1 ~ amendment1, family = Gamma(link = "log"), data = DFE1) # gamma.DFE1
glm.DFE1 <- glm(percent.em1 ~ amendment1, data = DFE1)
AIC(lglm.DFE1, gamma.DFE1, glm.DFE1)

lglm.DFE2 <- glm(percent.em2 ~ amendment2, data = DFE2, family = "binomial", weights = pupae2)
gamma.DFE2 <- glm(percent.em2 ~ amendment2, family = Gamma(link = "log"), data = DFE2) # gamma.DFE2
glm.DFE2 <- glm(percent.em2 ~ amendment2, data = DFE2)
AIC(lglm.DFE2, gamma.DFE2, glm.DFE2)


pois.DFE1 <- glm(days1 ~ amendment1, data = DFE1, family = poisson) 
qpois.DFE1 <- glm(days1 ~ amendment1, data = DFE1, family = quasipoisson) # nbin.DPR1
...
qpois.DFE2  <- glm(days2 ~ amendment2, data = DFE2, family = quasipoisson) # nbin.DPR1
...
Anova(lglm.DFE1)
gamma.DFE1 <- glm(percent.em1 ~ amendment1, family = Gamma(link = "log"), data = DFE1)
Anova(gamma.DFE1)
...
analysis_l_1<-lapply(delia.list[1], function(x){
...
analysis_l_2 <-lapply(delia.list[2], function(x){
glm_output <- glm(survival ~ amendment, data = x, family = poisson)
glm_output_anova<-Anova(glm_output)
return(glm_output_anova)
})
analysis_l_2
...
# Number of pupae retrieved (pupal survival)
P18 <- ggplot(DPR1, aes(amendment1, survival))+ 
  ...
  labs(x = "Treatment", y = "% Survival (larvae + pupae)")+
...
P22 <- ggplot(DFE1, aes(amendment1, percent_per_day1))+ 
  ...
  stat_summary(fun = mean, geom = "point", col = "white", shape = 12) +
  stat_summary(fun = mean, geom = "point", col = "white") + 
  labs(x = "Treatment", y = "Percent emergence of Delia radicum flies")+
  scale_fill_manual(values = c("#91BAD6", "#91BAD6","#91BAD6"))# Dark Sky Blue
P22 + ylim (0, 80)
...
P23 <- ggplot(DFE2, aes(amendment2, percent_per_day2))+ 
  geom_boxplot(aes(fill = amendment2), show.legend = FALSE) + 
  stat_summary(fun = mean, geom = "point", col = "white", shape = 12) +
  stat_summary(fun = mean, geom = "point", col = "white") + 
  labs(x = "Treatment", y = "Percent emergence of Delia radicum flies")+
...
# Time to emergence
P24 <- ggplot(DFE1, aes(amendment1, days1))+ 
  geom_boxplot(aes(fill = amendment1), show.legend = FALSE) + 
  stat_summary(fun = mean, geom = "point", col = "white", shape = 12) +
  stat_summary(fun = mean, geom = "point", col = "white") + 
  labs(x = "Treatment", y = "Time (days) to emergence of D. radicum flies")+
  scale_fill_manual(values = c("#91BAD6", "#91BAD6","#91BAD6"))# Dark Sky Blue
P24 + ylim (0, 15)
...
P25 <- ggplot(DFE2, aes(amendment2, days2))+ 
  geom_boxplot(aes(fill = amendment2), show.legend = FALSE) + 
  stat_summary(fun = mean, geom = "point", col = "white", shape = 12) +
  stat_summary(fun = mean, geom = "point", col = "white") + 
  labs(x = "Treatment", y = "Time (days) to emergence of D. radicum flies")+
...
AIC(qpois.PPR1, glm.PPR1, nbin.PPR1)
...
AIC(qpois.PPR2, glm.PPR2, nbin.PPR2)
...
nbin.output <- glm.nb(survival ~ amendment, data = xp) # xp can be anything
nbin.output_anova<-Anova(nbin.output)
return(nbin.output_anova)
})
